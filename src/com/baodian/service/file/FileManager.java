...
public class FileManager {
	...
	public String getRootPath(int userId) {
		if(rootPath == null) {
			String initPath = InitDataManager.upload[4];
			if(initPath.startsWith("/") || initPath.contains(":")) {//绝对路径
				rootPath = initPath;
			} else {//相对路径
				...
				while(initPath.startsWith("../")) {
					if(isWindows) {
						rootPath = rootPath.substring(0, rootPath.lastIndexOf(92));
					} else {
						rootPath = rootPath.substring(0, rootPath.lastIndexOf(47));
					}
					initPath = initPath.substring(3);
				}
				rootPath = rootPath + "/"+ initPath;
			}
		}
