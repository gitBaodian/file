...
function upLoad() {
	...
	$('#file_uploads').uploadify({
		...
		'fileSizeLimit': formatFileSize(index_user.uploadSize),
		'fileObjName': 'imgFile',
		'width': 108,
		//'progressData' : 'speed',
		//'debug': true,
	...
	$('#index_form').form('submit',{
		...
		success: function(data){
			...
			if(data.status == 0) {
				...
				//上传大小有变动
				if(index_user.uploadSize != data.uploadSize) {
					try {
						$('#file_uploads_dlg').dialog('close');
					} catch(err) {}
					index_user.uploadSize = data.uploadSize;
				}
				loadIndex(true);
				loadFile(1);
			} else {
